...
def train_autoencoder(gpu, args):
    ...
    def do_epoch(epoch, dataset, partition="train"):
        ...
        def do_minibatch(i, batch_data):
            ...
            if partition == "train":
                h_e3gnn, h_xformer, logits, bad_rows = model.module.forward_dist(
                    torch.Tensor(batch_data["raw_tokens"]).to(device),
                    torch.Tensor(batch_data["augmented_tokens"]).to(device),
                    torch.Tensor(batch_data["eclouds"]).to(torch.float).to(device),
                    tokenizer,
            ...
            y_next = torch.zeros_like(batch_data["augmented_tokens"]).to(device)
            y_next[:, :(batch_data["augmented_tokens"].shape[1] - 1)] = \
                torch.Tensor(batch_data["augmented_tokens"]).clone()[:, 1:]
        ...
        epoch_iter = DataLoader(dataset = dataset, #num_workers=4,
