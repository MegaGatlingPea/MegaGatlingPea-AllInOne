...
# from utils.chem import UniqueSmiles
disable_logger()
...
class e3gnn_smiles_clip_e2e(nn.Module):
    ...
    def encode_eclouds(self, eclouds: torch.Tensor) -> torch.Tensor:
        """
        Embeds eclouds into the latent space.

        """
        ecloud_emb = self.ecloud_encoder(eclouds).transpose(0, 1) # (b, s, 768)
        ecloud_emb = ecloud_emb.reshape(ecloud_emb.shape[0], -1) # (b, s*768)
        return self.eclouds_to_clip(ecloud_emb)

    def eclouds_to_2d_batch(
        self,
        eclouds_batch: torch.Tensor,
        tokenizer: TrieTokenizer,
        ...
        """
        h_clip = self.encode_eclouds(eclouds_batch) # (b, 128)
        if noise_scale > 0:
            h_clip += torch.normal(
                mean=torch.zeros_like(h_clip), std=noise_scale * torch.ones_like(h_clip)
            )
        h_token = self.point_clip_to_special_tokens(h_clip) # (b, 128)
        if do_suffix:
        ...
        # inject ecloud embeddings into the transformer.
    ...
    def ecloud_and_token_to_2d_batch(
        self,
        ...
        """
        h_ecloud = self.encode_eclouds(eclouds).repeat(num_to_gen, 1)
        h_smiles = self.encode_tokens(tokens, tokenizer).repeat(num_to_gen, 1)
        if noise_scale > 0:
            h_smiles += torch.normal(
                mean=torch.zeros_like(h_smiles), std=noise_scale * torch.ones_like(h_smiles)
            )
        point_clip_token = self.point_clip_to_special_tokens(h_ecloud)
    ...
    def decipher_eclouds_to_mols(
        ...
        Input:
            ...
            keep_special: whether to keep the special tokens.
        """
        if len(eclouds_batch.shape) == 3:
            eclouds_batch = eclouds_batch.unsqueeze(0)
        if batch_repeat > 1:
            eclouds_batch = eclouds_batch.repeat(batch_repeat, 1, 1, 1)
        ...
        for attempt in range(max_attempts):
            ...
            smiles_list = [Chem.MolToSmiles(m) for m in mols_list]
            ...
            if unique:
                ...
                if len(unique_smiles) >= total_gen:
                    break
    ...
    def forward_dist(
        ...
        with autocast(enabled=False, device_type="cuda"):
            h_e3gnn = self.encode_eclouds(eclouds) 
            ...
            try:
                ...
                print(
                    Ex,
                    raw_tokens.shape, # (b, s)
                    augmented_tokens.shape, # (b, s)
                    eclouds.shape, # (b, 32, 32, 32)
                    h_e3gnn.shape, # (b, 256)
                    h_smiles.shape, # (b, 256)
                )
                raise Ex
            point_clip_token = self.point_clip_to_special_tokens(h_e3gnn)
            smiles_clip_token = self.point_clip_to_special_tokens(h_smiles)
            clip_token = torch.where(
                (torch.rand((h_e3gnn.shape[0],), device=eclouds.device) > p_clip_emb_smi)
                .unsqueeze(-1)
                .repeat(1, point_clip_token.shape[-1]),
                point_clip_token,
                smiles_clip_token,
