...
def train_due(
    ...
    train_rec_indices, test_rec_indices = np.split(np.random.permutation(len(dataset)), [int(test_frac * len(dataset))])
    ...
    if not load_as is None:
        ...
        model.load_state_dict(read['model_state_dict'])
...
def trained_model(ckpt_path):
    ...
    feature_extractor = FCResNet(
        ...
        spectral_normalization=model_config['spectral_normalization'],
        coeff=model_config['coeff'],
        n_power_iterations=model_config['n_power_iterations'],
        dropout_rate=model_config['dropout_rate'],
    )

    gp = GP(
        num_outputs=model_config['num_outputs'],
        initial_lengthscale=model_config['initial_lengthscale'],
        initial_inducing_points=model_config['initial_inducing_points'],
